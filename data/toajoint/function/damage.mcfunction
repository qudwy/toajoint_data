execute as @a at @s if entity @s[tag=!uuid] run function toajoint:player
execute if entity @s[tag=!owner] run function toajoint:trident

tag @s add searching
execute as @a at @s if score @s a.uuid = @n[tag=searching] a.owner if score @s b.uuid = @n[tag=searching] b.owner if score @s c.uuid = @n[tag=searching] c.owner if score @s d.uuid = @n[tag=searching] c.owner run tag @s add owner
